...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    /* --- BEGIN: VoIP-GSM Bridge Support --- */
    /* 处理 incall_music_uplink=on/off 参数，强制启用/禁用上行混音用例 */
    ret = str_parms_get_str(parms, "incall_music_uplink", value, sizeof(value));
    if (ret >= 0) {
        ALOGD("%s: VoIP-GSM Bridge: incall_music_uplink=%s", __func__, value);
        
        if (strcmp(value, "on") == 0) {
            /* 开启上行混音：找到active的playback流并强制启用incall_music用例 */
            struct audio_usecase *uc = NULL;
            struct audio_usecase *playback_uc = NULL;
            
            /* 查找第一个active的PCM_PLAYBACK用例 */
            list_for_each(node, &adev->usecase_list) {
                uc = node_to_item(node, struct audio_usecase, list);
                if (uc->type == PCM_PLAYBACK && uc->stream.out && 
                    uc->stream.out->usecase != USECASE_INCALL_MUSIC_UPLINK &&
                    uc->stream.out->usecase != USECASE_INCALL_MUSIC_UPLINK2) {
                    playback_uc = uc;
                    ALOGD("%s: Found playback usecase: %d", __func__, uc->id);
                    break;
                }
            }
            
            if (playback_uc && playback_uc->stream.out) {
                /* 强制将此playback流的usecase改为incall_music_uplink */
                ALOGI("%s: Force-enabling INCALL_MUSIC_UPLINK for VoIP-GSM bridge", __func__);
                
                /* 调用voice_extn来设置incall_music用例 */
                if (voice_extn_check_and_set_incall_music_usecase(adev, playback_uc->stream.out) == 0) {
                    ALOGI("%s: Successfully activated INCALL_MUSIC_UPLINK usecase", __func__);
                    /* 重新选择设备以应用新的mixer路径 */
                    select_devices(adev, playback_uc->id);
                } else {
                    ALOGE("%s: Failed to activate INCALL_MUSIC_UPLINK usecase", __func__);
                }
            } else {
                ALOGW("%s: No active playback stream found for incall_music_uplink", __func__);
            }
        } else if (strcmp(value, "off") == 0) {
            /* 关闭上行混音：禁用incall_music用例 */
            ALOGI("%s: Disabling INCALL_MUSIC_UPLINK for VoIP-GSM bridge", __func__);
            /* 这里可以添加禁用逻辑，但通常停止播放AudioTrack就会自动清理 */
        }
    }
    /* --- END: VoIP-GSM Bridge Support --- */
